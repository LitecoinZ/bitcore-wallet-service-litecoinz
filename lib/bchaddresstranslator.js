...
BCHAddressTranslator.getAddressCoin = function(address) {
  try {
    ...
    try {
      var a= new Bitcore_['ltz'].Address(address);
...
BCHAddressTranslator.translate = function(addresses, to, from) {
  ...
  if (from == to) {
    ...
    ret =  _.map(addresses, function(x) {
      var bitcore = Bitcore_[from == 'legacy' ? 'btc' : 'ltz'];
      var orig = new bitcore.Address(x).toObject();

      if (to == 'cashaddr') {
        return Bitcore_['ltz'].Address.fromObject(orig).toCashAddress(true);
      } else if (to == 'copay') {
        return Bitcore_['ltz'].Address.fromObject(orig).toLegacyAddress();
      } else if (to == 'legacy') {
