 ...
 * @param {string} opts.coin[='ltz'] - The coin for this wallet (btc, ltz).
 ...
 * @param {string} opts.coin[='ltz'] - The expected coin for this wallet (btc, ltz).
...
WalletService.prototype._getUtxosForCurrentWallet = function(opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      ...
      coin = opts.coin;
      if (coin != 'ltz') return next();

      if (Utils.getAddressCoin(addressStrs[0]) == 'ltz')
        return next();

      // because some old LTZ walelts could have legacy addresses?
